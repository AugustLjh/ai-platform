  ...
  <div class="subagent-page">
    <section class="subagent-hero">
      <div>
        ...
        <h1>专家能力控制台</h1>
        <p>维护 capability definition、版本、发布状态和测试运行入口，让管理员直接完成 E3 控制面闭环。</p>
      </div>
      <div class="hero-actions">
        <button type="button" class="btn btn-secondary" @click="reloadAll">刷新</button>
        <button type="button" class="btn btn-primary" :disabled="!isAdmin" @click="openCreate">新增能力</button>
      </div>
    </section>
    ...
    <section class="subagent-grid">
      <div class="card list-card">
        <div class="section-head">
          ...
          <label class="toggle">
            <input v-model="includeArchived" type="checkbox" @change="loadSubagents" />
        ...
        <div v-else class="subagent-list">
          <button
            ...
            type="button"
            :class="['subagent-item', { active: selectedDefinitionId === item.definitionId }]"
            @click="selectSubagent(item.definitionId)"
          >
            <div class="subagent-item-top">
              <strong>{{ item.name }}</strong>
              <span :class="['status-pill', `status-${toneStatus(item.status)}`]">{{ item.status }}</span>
            </div>
            ...
            <div class="subagent-item-tags">
              ...
              <span v-if="item.hostAgentDefinitionId" class="mini-badge">host {{ agentNameById(item.hostAgentDefinitionId) }}</span>
              <span v-if="item.handoffPrompt" class="mini-badge">含 handoff</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div class="card editor-card">
        <div class="section-head">
          <div>
            <h2>{{ editingDefinitionId ? '编辑 Definition' : '新增 Definition' }}</h2>
            <p>Definition 保存 capability 基线；运行时行为由版本与 publication 决定。</p>
          </div>
          <div class="detail-actions">
            <button type="button" class="btn btn-secondary" @click="resetDefinitionForm">清空</button>
            <button
        ...
        <form class="editor-form" @submit.prevent="saveDefinition">
          <div class="field-row">
            <label class="field">
              <span>名称</span>
              <input v-model="definitionForm.name" class="input" maxlength="120" />
            </label>
            <label class="field">
              <span>Slug</span>
              <input v-model="definitionForm.slug" class="input" maxlength="120" placeholder="例如 code-reviewer" />
            </label>
          </div>

          <label class="field">
            <span>描述</span>
            <textarea v-model="definitionForm.description" class="input textarea" rows="3"></textarea>
          </label>
          ...
          <div class="field-row">
            ...
            <label class="field">
              <span>Model</span>
              <input v-model="definitionForm.model" class="input" placeholder="留空表示沿用 runtime 默认" />
            </label>
          </div>
          ...
          <div class="field-row">
            <label class="field">
              ...
              <select v-model="definitionForm.hostAgentDefinitionId" class="input">
          ...
          <label class="field">
            ...
            <textarea v-model="definitionForm.systemPrompt" class="input textarea tall-textarea" rows="8"></textarea>
          </label>
          ...
          <details class="advanced-panel">
            ...
            <div class="advanced-grid">
              <label class="field">
                ...
                <textarea v-model="definitionForm.configText" class="input textarea" rows="6"></textarea>
              </label>
              <label class="field">
                <span>Metadata JSON</span>
                <textarea v-model="definitionForm.metadataText" class="input textarea" rows="6"></textarea>
              </label>
              <label class="field">
                <span>Output Schema JSON</span>
                <textarea v-model="definitionForm.outputSchemaText" class="input textarea" rows="6"></textarea>
              </label>
              <label class="field">
                <span>Handoff Input Schema JSON</span>
                <textarea v-model="definitionForm.handoffInputSchemaText" class="input textarea" rows="6"></textarea>
              </label>
              <label class="field">
                <span>Tool Allowlist JSON</span>
                <textarea v-model="definitionForm.toolAllowlistText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>Skill Allowlist JSON</span>
                <textarea v-model="definitionForm.skillAllowlistText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>MCP Allowlist JSON</span>
                <textarea v-model="definitionForm.mcpAllowlistText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>Knowledge Policy JSON</span>
                <textarea v-model="definitionForm.knowledgePolicyText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>Review Policy JSON</span>
                <textarea v-model="definitionForm.reviewPolicyText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>Runtime Policy JSON</span>
                <textarea v-model="definitionForm.runtimePolicyText" class="input textarea" rows="5"></textarea>
              </label>
              <label class="field">
                <span>Publication Metadata JSON</span>
                <textarea v-model="definitionForm.publicationMetadataText" class="input textarea" rows="5"></textarea>
              </label>
            </div>
          </details>

          <div class="form-actions">
            <button type="submit" class="btn btn-primary" :disabled="savingDefinition || !isAdmin">
              {{ savingDefinition ? '保存中...' : (editingDefinitionId ? '保存 Definition' : '创建 Definition') }}
            </button>
          </div>
        </form>
      </div>
    ...
    <section v-if="controlPlane.definition" class="card detail-card">
      <div class="section-head">
        ...
        <div class="detail-actions">
          <button type="button" class="btn btn-secondary" :disabled="loadingDetail" @click="loadControlPlane(selectedDefinitionId)">
            重新载入
          </button>
          <button type="button" class="btn btn-secondary" :disabled="!selectedDefinitionId" @click="seedVersionFormFromDefinition">
            用当前定义填充新版本
          </button>
        </div>
      </div>
      ...
      <section class="summary-grid">
        <article class="summary-card">
          ...
          <strong>{{ controlPlane.publication?.versionNumber ? `v${controlPlane.publication.versionNumber}` : '未发布' }}</strong>
          <p>{{ controlPlane.publication ? publicationScopeLabel(controlPlane.publication.publicationScope) : '暂无 publication' }}</p>
        </article>
        <article class="summary-card">
        ...
        <article class="summary-card accent">
          ...
          <strong>{{ agentNameById(controlPlane.definition.hostAgentDefinitionId) }}</strong>
          <p>{{ controlPlane.governance?.compatibilityMode ? '兼容宿主覆盖已开启' : (controlPlane.definition.model || '模型未锁定') }}</p>
        </article>
      </section>
      ...
      <section v-if="controlPlane.governance" class="governance-strip">
        <article class="governance-card">
          ...
          <strong>{{ controlPlane.governance.isPublishedVersionLatest ? '已追平最新版本' : '当前发布落后最新版本' }}</strong>
          <p>
        ...
        <article class="governance-card">
          ...
          <strong>{{ controlPlane.governance.enabledAuthorizationCount }} / {{ controlPlane.governance.authorizationCount }}</strong>
          <p>active host agent 之外还有 {{ controlPlane.governance.inactiveAuthorizationCount }} 个待清理授权</p>
        </article>
        <article class="governance-card">
          <span class="summary-label">兼容模式</span>
          <strong>{{ controlPlane.governance.compatibilityMode ? '仍在使用' : '未启用' }}</strong>
          <p>{{ controlPlane.governance.hostAgentDefinitionId ? agentNameById(controlPlane.governance.hostAgentDefinitionId) : '未指定宿主覆盖' }}</p>
        </article>
      </section>
      ...
      <div v-if="governanceWarnings.length > 0" class="governance-warning-list">
        <article
          ...
          :key="warning.code"
          :class="['governance-warning', `severity-${warning.severity || 'info'}`]"
        >
      ...
      <div class="detail-grid">
        <div class="detail-panel">
          ...
          <div class="detail-table">
            <div><span>Status</span><strong>{{ controlPlane.publication?.status || '未发布' }}</strong></div>
            <div><span>Version</span><strong>{{ controlPlane.publication?.versionNumber ? `v${controlPlane.publication.versionNumber}` : '-' }}</strong></div>
            <div><span>Scope</span><strong>{{ publicationScopeLabel(controlPlane.publication?.publicationScope || controlPlane.definition.publicationScope) }}</strong></div>
            <div><span>Authorizations</span><strong>{{ controlPlane.publication?.authorizationCount || 0 }}</strong></div>
          </div>
          ...
          <form class="stack-form" @submit.prevent="savePublication">
            <div class="field-row">
              <label class="field">
                ...
                <select v-model="publicationForm.versionId" class="input">
                  ...
                  <option v-for="version in controlPlane.versions" :key="version.id" :value="version.id">
            ...
            <label class="field">
              <span>Publication Metadata JSON</span>
              <textarea v-model="publicationForm.metadataText" class="input textarea" rows="5"></textarea>
            </label>

            <div class="form-actions">
              <button type="submit" class="btn btn-primary" :disabled="busyAction === 'publication' || !isAdmin">
                {{ busyAction === 'publication' ? '发布中...' : '保存 Publication' }}
              </button>
            </div>
          </form>
        </div>
          ...
          <form class="stack-form" @submit.prevent="createTestRun">
            ...
            <div class="field-row">
              <label class="field">
                ...
                <input v-model="testRunForm.sessionId" class="input" placeholder="留空由后端自动处理" />
              </label>
            ...
            <label class="field">
              <span>Metadata JSON</span>
              <textarea v-model="testRunForm.metadataText" class="input textarea" rows="5"></textarea>
            </label>

            <div class="form-actions">
              <button type="submit" class="btn btn-primary" :disabled="busyAction === 'test-run' || !isAdmin">
      ...
      <div class="detail-grid secondary-grid">
        <div class="detail-panel">
          ...
          <form class="stack-form" @submit.prevent="createVersion">
            <div class="field-row">
              ...
              <label class="field">
                <span>Model</span>
                <input v-model="versionForm.model" class="input" placeholder="留空表示沿用默认" />
              </label>
            </div>

            <label class="field">
              <span>System Prompt</span>
              <textarea v-model="versionForm.systemPrompt" class="input textarea tall-textarea" rows="8"></textarea>
            </label>
            ...
            <details class="advanced-panel">
              ...
              <div class="advanced-grid">
                <label class="field">
                  <span>Config JSON</span>
                  <textarea v-model="versionForm.configText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Metadata JSON</span>
                  <textarea v-model="versionForm.metadataText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Output Schema JSON</span>
                  <textarea v-model="versionForm.outputSchemaText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Handoff Input Schema JSON</span>
                  <textarea v-model="versionForm.handoffInputSchemaText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Tool Allowlist JSON</span>
                  <textarea v-model="versionForm.toolAllowlistText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Skill Allowlist JSON</span>
                  <textarea v-model="versionForm.skillAllowlistText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>MCP Allowlist JSON</span>
                  <textarea v-model="versionForm.mcpAllowlistText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Knowledge Policy JSON</span>
                  <textarea v-model="versionForm.knowledgePolicyText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Review Policy JSON</span>
                  <textarea v-model="versionForm.reviewPolicyText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Runtime Policy JSON</span>
                  <textarea v-model="versionForm.runtimePolicyText" class="input textarea" rows="5"></textarea>
                </label>
                <label class="field">
                  <span>Publication Metadata JSON</span>
                  <textarea v-model="versionForm.publicationMetadataText" class="input textarea" rows="5"></textarea>
                </label>
              </div>
            </details>

            <div class="form-actions">
              <button type="submit" class="btn btn-primary" :disabled="busyAction === 'version' || !isAdmin">
        ...
        <div class="detail-panel">
          ...
          <div v-if="controlPlane.versions.length === 0" class="mini-empty">当前还没有版本。</div>
          <div v-else class="version-list">
            <article v-for="version in controlPlane.versions" :key="version.id" class="version-card">
              <div class="version-card-head">
                <div>
                  <strong>v{{ version.versionNumber }}</strong>
                  <span>{{ version.lifecycleStatus }} · {{ version.model || 'default model' }}</span>
                </div>
                <span v-if="version.isPublished" class="mini-badge accent-badge">当前发布</span>
              </div>
              <p class="version-copy">{{ truncate(version.systemPrompt, 180) }}</p>
              <div class="version-card-meta">
                <span>{{ Object.keys(version.outputSchema || {}).length > 0 ? '含 output schema' : '无 output schema' }}</span>
                <span>{{ Array.isArray(version.toolAllowlist) ? `${version.toolAllowlist.length} tools` : '0 tools' }}</span>
                <span>{{ formatTime(version.updatedAt) }}</span>
              </div>
              <div class="card-inline-actions">
                <button type="button" class="btn btn-secondary btn-inline" @click="copyVersionIntoForm(version)">复制到新版本表单</button>
                <button
                  type="button"
                ...
                >
                  {{ busyAction === `publish-${version.id}` ? '发布中...' : (version.isPublished ? '重新发布' : '发布该版本') }}
                </button>
              </div>
            </article>
          </div>
          ...
          <div class="authorization-section">
            <h4>Host Agent 授权</h4>
            <div v-if="controlPlane.authorizations.length === 0" class="mini-empty">当前没有 agent 授权这个 capability。</div>
            <div v-else class="authorization-list">
              <router-link
                v-for="item in controlPlane.authorizations"
                :key="item.authorizationId"
                :to="{ name: 'AgentExtensions', params: { id: item.agentDefinitionId } }"
...
const isAdmin = computed(() => String(authStore.user?.role || '').trim().toLowerCase() === 'admin')
const activeAgents = computed(() => agents.value.filter((item) => item.status === 'active'))
const enabledAuthorizationCount = computed(() => controlPlane.authorizations.filter((item) => item.status === 'enabled').length)
const latestVersionLabel = computed(() => controlPlane.versions[0]?.versionNumber ? `v${controlPlane.versions[0].versionNumber}` : '暂无版本')
const governanceWarnings = computed(() => Array.isArray(controlPlane.governance?.warnings) ? controlPlane.governance.warnings : [])
...
const normalizePublication = (raw = {}) => ({
  ...
  status: raw.status || 'active',
  metadata: parseJSON(raw.metadata, {}),
  authorizationCount: Number(raw.authorization_count || raw.authorizationCount || 0),
  createdAt: raw.created_at || raw.createdAt || null,
  updatedAt: raw.updated_at || raw.updatedAt || null
})
...
const normalizeGovernance = (raw = {}) => ({
  compatibilityMode: Boolean(raw.compatibility_mode || raw.compatibilityMode),
  hostAgentDefinitionId: raw.host_agent_definition_id || raw.hostAgentDefinitionId || '',
  latestVersionNumber: Number(raw.latest_version_number || raw.latestVersionNumber || 0),
  publishedVersionNumber: Number(raw.published_version_number || raw.publishedVersionNumber || 0),
  isPublishedVersionLatest: Boolean(raw.is_published_version_latest || raw.isPublishedVersionLatest),
  authorizationCount: Number(raw.authorization_count || raw.authorizationCount || 0),
  enabledAuthorizationCount: Number(raw.enabled_authorization_count || raw.enabledAuthorizationCount || 0),
  inactiveAuthorizationCount: Number(raw.inactive_authorization_count || raw.inactiveAuthorizationCount || 0),
...
const applyControlPlane = (raw = {}) => {
  controlPlane.definition = raw.definition ? normalizeSubagent(raw.definition) : null
  controlPlane.versions = Array.isArray(raw.versions) ? raw.versions.map(normalizeVersion) : []
  controlPlane.publication = raw.publication ? normalizePublication(raw.publication) : null
  controlPlane.authorizations = Array.isArray(raw.authorizations) ? raw.authorizations.map(normalizeAuthorization) : []
  controlPlane.governance = raw.governance ? normalizeGovernance(raw.governance) : null
}
...
const publicationScopeLabel = (scope) => String(scope || '').trim() === 'system_global' ? '系统发布' : '租户发布'
const governanceSeverityLabel = (severity) => ({
  warning: '警告',
  error: '风险',
  info: '提示'
}[String(severity || '').trim().toLowerCase()] || '提示')
...
const mergeDefinitionMetadata = () => {
  const metadata = parseJSONField(definitionForm.metadataText, {}, 'Metadata JSON')
...
const buildDefinitionPayload = () => ({
  ...
  output_schema: parseJSONField(definitionForm.outputSchemaText, {}, 'Output Schema JSON'),
  handoff_input_schema: parseJSONField(definitionForm.handoffInputSchemaText, {}, 'Handoff Input Schema JSON'),
  tool_allowlist: parseJSONField(definitionForm.toolAllowlistText, [], 'Tool Allowlist JSON'),
  skill_allowlist: parseJSONField(definitionForm.skillAllowlistText, [], 'Skill Allowlist JSON'),
  mcp_allowlist: parseJSONField(definitionForm.mcpAllowlistText, [], 'MCP Allowlist JSON'),
  knowledge_policy: parseJSONField(definitionForm.knowledgePolicyText, {}, 'Knowledge Policy JSON'),
  review_policy: parseJSONField(definitionForm.reviewPolicyText, {}, 'Review Policy JSON'),
  runtime_policy: parseJSONField(definitionForm.runtimePolicyText, {}, 'Runtime Policy JSON'),
  publication_metadata: parseJSONField(definitionForm.publicationMetadataText, {}, 'Publication Metadata JSON')
})
...
const buildVersionPayload = () => ({
  ...
  metadata: parseJSONField(versionForm.metadataText, {}, 'Version Metadata JSON'),
  output_schema: parseJSONField(versionForm.outputSchemaText, {}, 'Version Output Schema JSON'),
  handoff_input_schema: parseJSONField(versionForm.handoffInputSchemaText, {}, 'Version Handoff Input Schema JSON'),
  tool_allowlist: parseJSONField(versionForm.toolAllowlistText, [], 'Version Tool Allowlist JSON'),
  skill_allowlist: parseJSONField(versionForm.skillAllowlistText, [], 'Version Skill Allowlist JSON'),
  mcp_allowlist: parseJSONField(versionForm.mcpAllowlistText, [], 'Version MCP Allowlist JSON'),
  knowledge_policy: parseJSONField(versionForm.knowledgePolicyText, {}, 'Version Knowledge Policy JSON'),
  review_policy: parseJSONField(versionForm.reviewPolicyText, {}, 'Version Review Policy JSON'),
  runtime_policy: parseJSONField(versionForm.runtimePolicyText, {}, 'Version Runtime Policy JSON'),
  ...
  publication_metadata: parseJSONField(versionForm.publicationMetadataText, {}, 'Version Publication Metadata JSON')
})
...
const fillDefinitionForm = (definition) => {
  ...
  definitionForm.hostAgentDefinitionId = definition.hostAgentDefinitionId || definition.targetAgentDefinitionId || ''
  ...
  definitionForm.handoffInputSchemaText = formatJSON(definition.handoffInputSchema || {})
  definitionForm.toolAllowlistText = formatJSON(definition.toolAllowlist || [])
  definitionForm.skillAllowlistText = formatJSON(definition.skillAllowlist || [])
  definitionForm.mcpAllowlistText = formatJSON(definition.mcpAllowlist || [])
  definitionForm.knowledgePolicyText = formatJSON(definition.knowledgePolicy || {})
  ...
  definitionForm.publicationMetadataText = formatJSON(definition.publicationMetadata || {})
}

const fillPublicationForm = () => {
  publicationForm.versionId = controlPlane.publication?.versionId || controlPlane.versions[0]?.id || ''
  publicationForm.publicationScope = controlPlane.publication?.publicationScope || controlPlane.definition?.publicationScope || 'tenant'
  publicationForm.status = controlPlane.publication?.status || controlPlane.definition?.status || 'active'
  publicationForm.metadataText = formatJSON(controlPlane.publication?.metadata || controlPlane.definition?.publicationMetadata || {})
}
...
const seedVersionFormFromDefinition = () => {
  ...
  versionForm.publish = true
  versionForm.publicationScope = controlPlane.publication?.publicationScope || definition.publicationScope || 'tenant'
  versionForm.publicationStatus = controlPlane.publication?.status || definition.status || 'active'
  ...
  versionForm.handoffInputSchemaText = formatJSON(definition.handoffInputSchema || {})
  ...
  versionForm.knowledgePolicyText = formatJSON(definition.knowledgePolicy || {})
  ...
  versionForm.publicationMetadataText = formatJSON(controlPlane.publication?.metadata || definition.publicationMetadata || {})
}
...
const copyVersionIntoForm = (version) => {
  ...
  versionForm.publicationScope = version.publicationScope || controlPlane.publication?.publicationScope || 'tenant'
  versionForm.publicationStatus = version.publicationStatus || controlPlane.publication?.status || 'active'
  ...
  versionForm.handoffInputSchemaText = formatJSON(version.handoffInputSchema || {})
  ...
  versionForm.publicationMetadataText = formatJSON(controlPlane.publication?.metadata || {})
}

const fillTestRunForm = () => {
  testRunForm.versionId = controlPlane.publication?.versionId || controlPlane.versions[0]?.id || ''
  testRunForm.agentDefinitionId = controlPlane.definition?.hostAgentDefinitionId || controlPlane.definition?.targetAgentDefinitionId || ''
...
const loadSubagents = async () => {
  ...
  try {
    ...
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '加载专家能力失败'
  } finally {
...
const loadControlPlane = async (definitionId) => {
  ...
  try {
    ...
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '加载 control plane 失败'
  } finally {
...
const saveDefinition = async () => {
  if (!definitionForm.name.trim()) {
    toastStore.showToast({ type: 'error', message: '请先填写 capability 名称' })
    return
  }
  ...
  try {
    ...
    toastStore.showToast({ type: 'success', message: isUpdate ? 'Definition 已保存' : 'Definition 已创建' })
    await loadSubagents()
    await selectSubagent(saved.definitionId)
  } catch (error) {
    console.error('Failed to save subagent definition:', error)
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '保存 definition 失败'
...
const deleteDefinition = async () => {
  ...
  const confirmed = window.confirm(`确认删除专家能力“${definitionForm.name || editingDefinitionId.value}”吗？`)
  if (!confirmed) return
  ...
  try {
    ...
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '删除专家能力失败'
    toastStore.showToast({ type: 'error', message: errorMessage.value })
  } finally {
    busyAction.value = ''
  }
...
const createVersion = async () => {
  ...
  errorMessage.value = ''
  try {
    await subagentsAPI.createSubagentVersion(selectedDefinitionId.value, buildVersionPayload())
    ...
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '创建版本失败'
    toastStore.showToast({ type: 'error', message: errorMessage.value })
  } finally {
    busyAction.value = ''
  }
...
const savePublication = async () => {
  ...
  try {
    await subagentsAPI.updateSubagentPublication(selectedDefinitionId.value, {
      ...
      publication_metadata: parseJSONField(publicationForm.metadataText, {}, 'Publication Metadata JSON')
    })
    toastStore.showToast({ type: 'success', message: 'Publication 已更新' })
    await loadControlPlane(selectedDefinitionId.value)
  } catch (error) {
    console.error('Failed to update publication:', error)
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '更新 publication 失败'
    toastStore.showToast({ type: 'error', message: errorMessage.value })
  } finally {
    busyAction.value = ''
  }
...
const publishVersion = async (version) => {
  ...
  try {
    await subagentsAPI.updateSubagentPublication(selectedDefinitionId.value, {
      ...
      publication_scope: controlPlane.publication?.publicationScope || controlPlane.definition?.publicationScope || 'tenant',
      status: 'active',
      publication_metadata: controlPlane.publication?.metadata || controlPlane.definition?.publicationMetadata || {}
    })
    toastStore.showToast({ type: 'success', message: `v${version.versionNumber} 已发布` })
    await loadControlPlane(selectedDefinitionId.value)
  } catch (error) {
    console.error('Failed to publish version:', error)
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '发布版本失败'
    toastStore.showToast({ type: 'error', message: errorMessage.value })
  } finally {
    busyAction.value = ''
  }
...
const createTestRun = async () => {
  ...
  errorMessage.value = ''
  try {
    const { data } = await subagentsAPI.createSubagentTestRun(selectedDefinitionId.value, {
    ...
    if (data?.id) {
      await router.push({ name: 'AgentRunDetail', params: { run_id: data.id } })
    }
  } catch (error) {
    console.error('Failed to create test run:', error)
    errorMessage.value = error?.response?.data?.error || error?.response?.data?.detail || error?.message || '创建测试运行失败'
    toastStore.showToast({ type: 'error', message: errorMessage.value })
  } finally {
    busyAction.value = ''
  }
...
onMounted(async () => {
  await loadAgents()
  await loadSubagents()
  if (!selectedDefinitionId.value && String(route.query.subagent || '').trim()) {
    await selectSubagent(String(route.query.subagent || '').trim())
  }
})

watch(() => route.query.subagent, async (value, previousValue) => {
  const next = String(value || '').trim()
  const previous = String(previousValue || '').trim()
  if (!next || next === previous || next === selectedDefinitionId.value) return
