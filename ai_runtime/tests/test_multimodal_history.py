...
def test_history_deserializer_preserves_content_parts():
    history = chat_helpers.deserialize_history(
        [
            {
                "role": "user",
                "content": "",
...
def test_materialize_request_user_parts_for_candidate_uses_attachment_text_or_file_id():
    ...
    materialized = chat_helpers.materialize_request_user_parts_for_candidate(
        candidate, request_user_parts, upload_context
    )
...
@pytest.mark.asyncio
async def test_resolve_llm_candidates_rejects_text_only_model_for_image_request(monkeypatch):
    class DummyLLM:
    ...
    monkeypatch.setattr(chat_helpers, "_get_governance_settings", fake_governance_settings)
    monkeypatch.setattr(chat_helpers, "_get_model_row", fake_get_model_row)
    monkeypatch.setattr(chat_helpers, "_create_llm_instance", fake_create_llm_instance)

    try:
        await chat_helpers.resolve_llm_candidates(
            tenant_id="tenant-1",
            user_id="user-1",
...
@pytest.mark.asyncio
async def test_resolve_llm_candidates_rejects_protocol_without_media_adapter(monkeypatch):
    class DummyLLM:
        capabilities = {
    ...
    monkeypatch.setattr(
        chat_helpers,
        "_build_model_candidate",
        lambda model_row, source: {**model_row, "llm": DummyLLM(), "source": source},
    )
...
@pytest.mark.asyncio
async def test_resolve_llm_candidates_builds_database_model_candidate(monkeypatch):
    class DummyLLM:
        capabilities = {
    ...
    monkeypatch.setattr(chat_helpers, "_get_governance_settings", fake_governance_settings)
    monkeypatch.setattr(chat_helpers, "_get_model_row", fake_get_model_row)
    monkeypatch.setattr(chat_helpers, "_create_llm_instance", lambda model_row: DummyLLM())

    resolution = await chat_helpers.resolve_llm_candidates(
        tenant_id="tenant-1",
        user_id="user-1",
        knowledge_base_id=None,
        route_scene="chat",
